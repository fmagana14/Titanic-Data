...
const getMedianFare = (data) => {
  const fares = data
    .filter(passenger => passenger.fields.fare !== undefined)
    .map(passenger => passenger.fields.fare)
    .sort((a, b) => a - b); // Step 2: Sort fares in ascending order
  
  const len = fares.length;
  if (len === 0) return 0; // Handle case with no fares
  
  // Step 3: Calculate the median
  if (len % 2 === 1) {
    // Odd number of fares, return the middle one
    return fares[Math.floor(len / 2)];
  } else {
    // Even number of fares, return the average of the two middle ones
    const mid1 = fares[len / 2 - 1];
    const mid2 = fares[len / 2];
    return (mid1 + mid2) / 2;
  }
};
...
const getAverageAgeByGender = (data, gender) => {
  const ages = data
    .filter(
      (passenger) =>
        passenger.fields.age !== undefined && passenger.fields.sex === gender
    )
    .map((passenger) => passenger.fields.age);
